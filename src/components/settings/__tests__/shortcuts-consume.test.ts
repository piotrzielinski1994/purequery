...
// R6b consume guard (TC-015): purequery deleted its local shortcut-row.tsx +
// shortcuts-section.tsx and resolves ShortcutsSection from @pziel/pureui with a
// scope-grouped `groups` prop, while keeping its own registry.ts (incl.
// ShortcutScope) + resolve.ts untouched. Reads the source tree off disk.

const testDir = dirname(fileURLToPath(import.meta.url));
const settingsDir = resolve(testDir, "..");
const shortcutsLibDir = resolve(testDir, "../../../lib/shortcuts");
const settingsRoutePath = resolve(testDir, "../../../routes/settings.tsx");

describe("purequery consumes the pureui shortcuts UI (TC-015)", () => {
  ...
  it("should resolve ShortcutsSection from @pziel/pureui at the render site", () => {
    const source = readFileSync(settingsRoutePath, "utf8");
  ...
  it("should keep its scope-grouped rendering by passing a groups prop", () => {
    const source = readFileSync(settingsRoutePath, "utf8");

    expect(source).toMatch(/groups=\{groups\}/);
    expect(source).toContain("SCOPE_ORDER");
    expect(source).toContain("SCOPE_LABELS");
  });
