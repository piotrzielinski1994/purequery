...
// TC-012 (R13 Task 4). purequery must CONSUME the hoisted theme engine from
// @pziel/pureui and delete its two local copies (apply-vars.ts + overrides.ts).
// This is a STATIC guard - it reads the actual source tree off disk (like
// pureui's updater/__tests__/purity.test.ts) rather than trusting a mock. We
// assert observable facts: the local module files are gone, no src file still
// imports them by their @/ alias, and the app-side catalog (theme-defaults.ts)
// is untouched (the token arrays + defaults stay app-side, pureui carries no
// catalog).

const testDir = dirname(fileURLToPath(import.meta.url));
const srcDir = resolve(testDir, "../../.."); // .../purequery/src
...
describe("purequery consumes the pureui theme engine (TC-012)", () => {
