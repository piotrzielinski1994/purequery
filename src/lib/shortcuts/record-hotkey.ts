...
// The physical key the combo should record, mirroring the matcher so a recorded
// hotkey is exactly what the matcher will later fire on. macOS Option composes the
// key into a special char (Opt+P -> "π", Opt+- -> "–") while event.code still
// reports the physical key, so for anything but an ASCII letter we prefer
...
function physicalKey(event: KeyEventLike): string | null {
  ...
  // isModifierKey's predicate narrows the else-branch to `never`; read it as a
...
// special char the registry rejects as unknown, so a recorded Cmd+Opt+P silently
...
export function useRecordHotkey(options: RecordHotkeyOptions): RecordHotkeyApi {
  ...
  // Mirror the latest callbacks into a ref so the keydown listener (bound once per
  // recording session) always calls the current onRecord/onCancel without
